...
%figuresdir = '‎Dropbox/Q2_2018_19_drop/TFG/Thesis/Images/NEDC';
...
% grid on;
% if save==1
%     saveas(gcf,strcat(figuresdir,'power_output'),'png');
% else
% end
...
% figure(12);
% hold on;
% plot(slip_left.time/60, slip_left.signals.values(:,1), 'b', 'LineWidth', 1.5);
% plot(slip_left.time/60, traction_correction_l.signals.values(:,1),'r','LineWidth', 1.5);
% legend('Left wheel slip ratio', 'Traction control intervention Left');
% xlim([0 2]);
% ylim([-0.5 0.5]);
% ylabel('Force [N]');
